...
MONGO_ADDRESS = os.environ.get("MONGO_ADDRESS", "mongodb://127.0.0.1:6567")
...
def decode(name, available_values, modbus_response):
    if len(available_values) > 1:
        ...
        try:
            value = [k['value'] for k in available_values if k['count'] == modbus_response][0]
        except IndexError:
        ...
        if name == "UNBALANCE AIRFLOW SELECTION":
            value = modbus_response - (2 ** 16) if modbus_response > 15 else modbus_response
            print(value)
        elif name in (
            ...
            value = (modbus_response - (2 ** 16)) / 10 if modbus_response > 500 else modbus_response / 10
        else:
...
def retrieve():
    ...
    with connect_modbus() as client:
        domeo_data = dict(coils=client.read_coils(address=0, count=17).bits,
    ...
    for name, commands in tqdm(modbus_commands.items()):
        for command in commands:
            ...
            data.append({'name': command['description'], 'register': response_data, 'value': value, 'unit': unit})
    return data
